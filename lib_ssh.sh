...
install_ssh_key() {
    ...
    # 1. Handle "Host ID Changed" error
    ...
    # Print the output for transparency
    echo "$OUTPUT"

    # 2. Analyze the result
    if [ $EXIT_CODE -eq 0 ]; then
        if [[ "$OUTPUT" == *"All keys were skipped"* ]]; then
             echo -e "${GREEN}SSH Key already exists (Skipped save).${NC}"
        else
    ...
    # Return success (0) even if skipped, because the goal "have key on server" is met
    return $EXIT_CODE
}