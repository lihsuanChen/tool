...
optimize_docker_storage() {
    ...
    log_step "OPTIMIZE" "Moving Docker storage to ${NEW_STORAGE_DIR} on ${TARGET_IP}..."

    ssh "${REMOTE_USER}@${TARGET_IP}" "bash -s" << EOF
    set -e

    # 1. DETECT CURRENT LOCATION (The Fix)
    # We ask Docker where the data root is currently set.
    # Likely returns: /dct_builds/docker (based on your readme)
    ...
    sudo systemctl stop docker.socket # Ensure socket is also down
    sleep 3
